...
def loadImages(N=100):
    from scipy import misc
    def load_pics(folder,n):
        imgs = []
        for i in range(n):
            img = misc.imread(folder+"img_{:1}.jpg".format(i+1))
            ch = img[:,:,0]
            imgs.append(ch)
    ...
    base = "images/"
    trainpic = load_pics(base+"imgTrn/", 100)
    testpic = load_pics(base + "imgTst/", 100)
    ntrain, width, height = trainpic.shape

    xtrain = (trainpic/np.float32(255)).reshape(200, width, height, 1)
    xtest = (testpic/np.float32(255)).reshape(200, width, height, 1)
...
xtrain, ytrain, xtest, ytest, width, height = loadImages(50)
